...
class TestInferirAvanceSinCascada:
    ...
    def test_etapa_inferida_tiene_datos_reales(self, db_session):
        ...
        assert row.oficio_correo == "Conformidad recibida", f"oficio_correo debe ser subject, obtuvo {row.oficio_correo!r}"
...
# ---------------------------------------------------------------------------
# T-INF-15  observaciones con resumen del correo (asunto + extracto body_clean)
# ---------------------------------------------------------------------------

class TestObservacionesConResumen:
    """Verifica que las observaciones de etapas inferidas incluyen asunto y
    extracto del body_clean del correo, manteniendo el prefijo INGESTA_INFER
    intacto para que la reversa (revertir_avance) siga funcionando.
    """

    def test_construir_observaciones_con_asunto_y_body(self):
        """Con asunto y body_clean, observaciones contiene el extracto del body (el asunto va en oficio_correo, no aquí)."""
        from app.services.inferencia_etapas import _construir_observaciones
        obs = _construir_observaciones(42, "Conformidad del servicio", "Estimado señor, adjunto la conformidad firmada.")
        assert obs.startswith("[INGESTA_INFER corr=42]"), f"Prefijo incorrecto: {obs!r}"
        assert "Estimado señor" in obs, f"Extracto del body no incluido: {obs!r}"
        # El asunto NO va en observaciones cuando hay body (va en oficio_correo)
        assert "Conformidad del servicio" not in obs, f"Asunto no debe estar en observaciones con body: {obs!r}"
        # Sin separador — ya que el asunto no está en observaciones
        assert " — " not in obs, f"Separador — no debe aparecer en el nuevo formato: {obs!r}"

    def test_construir_observaciones_sin_body_solo_asunto(self):
        """Sin body_clean, observaciones incluye solo el asunto."""
        from app.services.inferencia_etapas import _construir_observaciones
        obs = _construir_observaciones(7, "Orden de servicio N° 023", None)
        assert obs.startswith("[INGESTA_INFER corr=7]"), f"Prefijo incorrecto: {obs!r}"
        assert "Orden de servicio N° 023" in obs, f"Asunto no incluido: {obs!r}"
        assert " — " not in obs, f"Separador — no debe aparecer sin body: {obs!r}"

    def test_construir_observaciones_sin_asunto_ni_body(self):
        """Sin asunto ni body, cae al texto genérico pero mantiene el prefijo."""
        from app.services.inferencia_etapas import _construir_observaciones
        obs = _construir_observaciones(1, None, None)
        assert obs.startswith("[INGESTA_INFER corr=1]"), f"Prefijo incorrecto: {obs!r}"
        assert "Inferido automáticamente" in obs, f"Texto genérico no encontrado: {obs!r}"

    def test_construir_observaciones_body_limpia_saltos_de_linea(self):
        """Los saltos de línea del body_clean se reemplazan por espacio."""
        from app.services.inferencia_etapas import _construir_observaciones
        body = "Línea uno.\nLínea dos.\r\nLínea tres."
        obs = _construir_observaciones(5, "Asunto", body)
        assert "\n" not in obs, f"Observaciones no debe tener \\n: {obs!r}"
        assert "\r" not in obs, f"Observaciones no debe tener \\r: {obs!r}"

    def test_construir_observaciones_body_truncado_a_160_chars(self):
        """body_clean largo se trunca a 160 chars + '…'."""
        from app.services.inferencia_etapas import _construir_observaciones
        body = "A" * 200
        obs = _construir_observaciones(3, "Asunto", body)
        # El extracto en observaciones debe ser 160 "A"s + "…"
        extracto_esperado = "A" * 160 + "…"
        assert extracto_esperado in obs, f"Extracto truncado no encontrado: {obs!r}"

    def test_construir_observaciones_body_exacto_160_sin_elipsis(self):
        """body_clean de exactamente 160 chars NO agrega '…'."""
        from app.services.inferencia_etapas import _construir_observaciones
        body = "B" * 160
        obs = _construir_observaciones(3, "Asunto", body)
        assert "…" not in obs, f"No debe haber elipsis con body de 160 chars: {obs!r}"
        assert "B" * 160 in obs

    def test_observaciones_etapa_inferida_incluye_asunto(self, db_session):
        """La etapa inferida con inferir_avance_correo tiene asunto en observaciones."""
        from app.services.inferencia_etapas import inferir_avance_correo
        from sqlalchemy import select as sa_select

        proceso = _make_proceso(db_session)

        class _CorreoConResumen:
            id = 801
            subject = "Conformidad del servicio de limpieza"
            sender_name = "Luis Mamani"
            numero_oficio = "OF-2025-088"
            fecha_documento = datetime.date(2025, 10, 1)
            received_at = None
            body_clean = "Estimado señor director, adjunto la conformidad firmada por el área usuaria."

        inferir_avance_correo(
            db_session, proceso.id, _CorreoConResumen(),
            [_make_doc("CONFORMIDAD", 0.9)]
        )
        ...
        assert len(rows) == 1
        obs = rows[0].observaciones
        assert obs is not None
        # La marca INGESTA_INFER debe estar presente (revertir_avance depende de esto)
        assert "[INGESTA_INFER corr=801]" in obs, f"Marca INGESTA_INFER no encontrada: {obs!r}"
        # El extracto del body debe estar incluido en observaciones
        assert "Estimado señor director" in obs, f"Extracto del body no incluido: {obs!r}"
        # El asunto NO va en observaciones cuando hay body (va en oficio_correo)
        assert "Conformidad del servicio de limpieza" not in obs, f"Asunto no debe estar en observaciones con body: {obs!r}"
        # Verificar que oficio_correo tiene el asunto
        row = rows[0]
        assert row.oficio_correo == "Conformidad del servicio de limpieza", f"oficio_correo debe ser el asunto: {row.oficio_correo!r}"

    def test_observaciones_etapa_inferida_sin_body_clean(self, db_session):
        """Si body_clean es None, observaciones tiene solo asunto (sin separador —)."""
        from app.services.inferencia_etapas import inferir_avance_correo
        from sqlalchemy import select as sa_select

        proceso = _make_proceso(db_session)

        class _CorreoSinBody:
            id = 802
            subject = "Cotización para servicio de vigilancia"
            sender_name = "Rosa Flores"
            numero_oficio = "OF-2025-012"
            fecha_documento = datetime.date(2025, 5, 10)
            received_at = None
            body_clean = None

        inferir_avance_correo(
            db_session, proceso.id, _CorreoSinBody(),
            [_make_doc("COTIZACION", 0.9)]
        )
        ...
        obs = rows[0].observaciones
        assert obs is not None
        assert "[INGESTA_INFER corr=802]" in obs, f"Marca no encontrada: {obs!r}"
        assert "Cotización para servicio de vigilancia" in obs, f"Asunto no incluido: {obs!r}"
        assert " — " not in obs, f"Separador — no debe aparecer sin body: {obs!r}"

    def test_revertir_avance_sigue_funcionando_con_nuevo_formato(self, db_session):
        """revertir_avance detecta la marca corr=<id> en el nuevo formato de observaciones."""
        from app.services.inferencia_etapas import inferir_avance_correo, revertir_avance
        from sqlalchemy import select as sa_select

        proceso = _make_proceso(db_session)

        class _CorreoConBody:
            id = 901
            subject = "Conformidad final del servicio"
            sender_name = "Test Revisor"
            numero_oficio = "OF-2025-100"
            fecha_documento = datetime.date(2025, 11, 15)
            received_at = None
            body_clean = "Texto del cuerpo del correo con información relevante."

        inferir_avance_correo(
            db_session, proceso.id, _CorreoConBody(),
            [_make_doc("CONFORMIDAD", 0.9)]
        )

        # Verificar que la etapa fue creada
        rows_antes = db_session.execute(
            sa_select(EtapaRegistro).where(
                EtapaRegistro.proceso_id == proceso.id,
                EtapaRegistro.codigo_etapa == "E23",
            )
        ).scalars().all()
        assert len(rows_antes) == 1, "E23 debe existir antes de revertir"

        # Revertir usando la marca corr=901
        n = revertir_avance(db_session, proceso.id, correo_id=901)
        assert n == 1, f"Debe revertir exactamente 1 fila, revirtió {n}"

        # Verificar que la etapa fue eliminada
        rows_despues = db_session.execute(
            sa_select(EtapaRegistro).where(
                EtapaRegistro.proceso_id == proceso.id,
                EtapaRegistro.codigo_etapa == "E23",
            )
        ).scalars().all()
        assert len(rows_despues) == 0, "E23 debe haberse revertido"
